...
1. `composer require crystoline/laravel-redlock`
2. Add `Crystoline\RedLock\RedLockServiceProvider::class,` to the `providers` array in config/app.php
3. Enjoy!
...
The `Crystoline\RedLock\Traits\QueueWithoutOverlap` trait provides this functionality with very few changes to your job. Usually only two changes are necessary.

1. `use Crystoline\RedLock\Traits\QueueWithoutOverlap` as a trait
2. Change the `handle()` method to `handleSync()`

```php
use Crystoline\RedLock\Traits\QueueWithoutOverlap;
...
If you find a bug or want to contribute to the code or documentation, you can help by submitting an [issue](https://github.com/crystoline/laravel-redlock/issues) or a [pull request](https://github.com/crystoline/laravel-redlock/pulls).
